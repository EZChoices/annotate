(function (global) {
  ...
  // ------------------------------
  // Helpers from codex/test-qa-scoring-and-injection
  // ------------------------------
  function normalizeClip(item, index) {
    const clipId =
      item && (item.clipId || item.asset_id || item.id || item.clip_id);
    const language =
      (item && (item.language || item.language_hint || item.locale)) ||
      "unknown";
    return {
  ...
  function selectGoldClips(manifest) {
    ...
    const goldClips = manifest.items.filter(
      (item) => item && item.is_gold === true
    );
  ...
  function computeMetrics(clips, context) {
    ...
    const warningCount = Array.isArray(lint.warnings)
    ...
    const accuracyPenalty = Math.min(
      1,
      errorCount * 0.25 + translationGaps * 0.12
    );
    const consistencyPenalty = Math.min(
      1,
      warningCount * 0.12 + codeSwitchIssues * 0.18
    );
    ...
    if (translationGaps) {
      summaryBits.push(
        `${translationGaps} translation gap${translationGaps === 1 ? "" : "s"}`
      );
    }
    if (codeSwitchIssues) {
      summaryBits.push(
        `${codeSwitchIssues} code-switch flag${
          codeSwitchIssues === 1 ? "" : "s"
        }`
      );
    }

    const hasSubmission = !!(annotations && annotations.clip);
    const reviewedClips = hasSubmission ? Math.min(totalGoldClips, 1) : 0;
  ...
  // ------------------------------
  // Helpers from main (storage + scoring)
  // ------------------------------
  ...
  function scoreCodeSwitchF1(predSpans, goldSpans, toleranceMs) {
    ...
    golds.forEach((gold) => {
      ...
      preds.forEach((pred, idx) => {
        ...
        const score =
          Math.abs(pred.start - gold.start) + Math.abs(pred.end - gold.end);
        if (score < bestScore) {
          bestScore = score;
          best = pred;
          bestIdx = idx;
      ...
      if (best && bestIdx >= 0) {
        ...
        matches.push({
          gold,
          pred: best,
          deltaStart: best.start - gold.start,
          deltaEnd: best.end - gold.end,
        });
      }
    });
        ...
        .map((seg) => {
          ...
          const end =
            seg.end != null
              ? toNumber(seg.end)
              : seg.duration != null
              ? toNumber(seg.duration) + toNumber(seg.start)
              : NaN;
          const speaker =
            seg.speaker || seg.spk || seg.label || seg.speaker_id || null;
  ...
  function scoreDiarizationMAE(predRTTM, goldRTTM, options) {
    const penalty =
      options && Number.isFinite(options.penalty)
        ? options.penalty
      ...
      return input
        .map((cue) => {
          ...
          const start = toNumber(
            cue.start != null ? cue.start : cue.startTime || cue.start_seconds
          );
          const end = toNumber(
            cue.end != null ? cue.end : cue.endTime || cue.end_seconds
          );
  ...
  function computeDurations(cues) {
    ...
    return cues
      .map((cue) => Math.max(0, toNumber(cue.end) - toNumber(cue.start)))
      .filter((dur) => Number.isFinite(dur) && dur > 0);
  }
  ...
  function stddev(values) {
    ...
    const variance =
      list.reduce((acc, v) => acc + Math.pow(v - mean, 2), 0) / list.length;
  ...
  function scoreTranslationStats(translationVTT, referenceVTT) {
    ...
    const completeness =
      referenceTokens > 0
        ? translationTokens / referenceTokens
        : translationTokens > 0
        ? 1
        : 0;
    const correctness =
      referenceTokens > 0
        ? 1 -
          Math.min(
            1,
            Math.abs(translationTokens - referenceTokens) /
              Math.max(referenceTokens, 1)
          )
        : 1;
    return {
      completeness,
  ...
  function computeQAResult(pred, gold, options) {
    ...
    const predSpans =
      predicted.codeSwitchSpans ||
      predicted.code_switch_spans ||
      predicted.spans ||
      [];
    const goldSpans =
      goldData.codeSwitchSpans || goldData.code_switch_spans || goldData.spans || [];
    const codeswitch = scoreCodeSwitchF1(
      predSpans,
      goldSpans,
      thresholds.codeswitchToleranceMs
    );

    const predDiar =
      predicted.diarization ||
      predicted.diarization_rttm ||
      predicted.diarSegments ||
      predicted.rttm ||
      [];
    const goldDiar =
      goldData.diarization || goldData.diarization_rttm || goldData.rttm || [];
    const diarization = scoreDiarizationMAE(predDiar, goldDiar, {
      penalty: thresholds.diarizationPenaltySec,
    });

    const transcriptVTT =
      predicted.transcript ||
      predicted.transcript_vtt ||
      predicted.transcriptVTT ||
      "";
    const translationVTT =
      predicted.translation ||
      predicted.translation_vtt ||
      predicted.translationVTT ||
      "";
    const goldTranscript =
      goldData.transcript ||
      goldData.transcript_vtt ||
      goldData.transcriptVTT ||
      goldData.source_transcript_vtt ||
      "";
    const goldTranslation =
      goldData.translation ||
      goldData.translation_vtt ||
      goldData.translationVTT ||
      "";

    const cueStats = scoreCueStats(transcriptVTT, translationVTT);
    const translationStats = scoreTranslationStats(
      translationVTT,
      goldTranslation || goldTranscript
    );
    ...
    const diarScore =
      ...
      Math.min(
        1,
        (diarization.mae || 0) / Math.max(thresholds.diarizationMAE, 0.0001)
      );
    const cueScore = computeScoreFromDiff(
      cueStats.targetDiffSec,
      thresholds.cueTargetDiff
    );
    const translationScore = clamp01(
      (translationStats.completeness + translationStats.correctness) / 2
    );
    const consistencyScore = average(
      [diarScore, cueScore, translationScore].filter((v) => Number.isFinite(v))
    );
    const overallScore = average(
      [accuracyScore, diarScore, cueScore, translationScore].filter((v) =>
        Number.isFinite(v)
      )
    );
    ...
    if (
      parseVttCues(goldTranslation || "").length ||
      parseVttCues(goldTranscript || "").length
    ) {
      checks.push(
        (translationStats.completeness || 0) >= thresholds.translationCompleteness
      );
      checks.push(
        (translationStats.correctness || 0) >= thresholds.translationCorrectness
      );
    }
  ...
  function aggregateEntries(entries) {
    ...
    const codeswitchValues = metrics
      .map((m) =>
        m.codeswitch && Number.isFinite(m.codeswitch.f1)
          ? m.codeswitch.f1
          : null
      )
      .filter((v) => v != null);
    const diarValues = metrics
      .map((m) =>
        m.diarization && Number.isFinite(m.diarization.mae)
          ? m.diarization.mae
          : null
      )
      .filter((v) => v != null);
    const cueValues = metrics
      .map((m) =>
        m.cues && Number.isFinite(m.cues.avgCueLengthSec)
          ? m.cues.avgCueLengthSec
          : null
      )
      .filter((v) => v != null);
    const cueDiffValues = metrics
      .map((m) =>
        m.cues && Number.isFinite(m.cues.targetDiffSec)
          ? m.cues.targetDiffSec
          : null
      )
      .filter((v) => v != null);
    const translationCompletenessValues = metrics
      .map((m) =>
        m.translation && Number.isFinite(m.translation.completeness)
          ? m.translation.completeness
          : null
      )
      .filter((v) => v != null);
    const translationCorrectnessValues = metrics
      .map((m) =>
        m.translation && Number.isFinite(m.translation.correctness)
          ? m.translation.correctness
          : null
      )
      .filter((v) => v != null);
    const translationCharRatioValues = entries
      .map((entry) => {
        const qa = entry.qa || {};
        if (Number.isFinite(qa.translation_char_ratio))
          return qa.translation_char_ratio;
        const cues =
          entry.metrics && entry.metrics.cues ? entry.metrics.cues : null;
        return cues && Number.isFinite(cues.translationCompleteness)
          ? cues.translationCompleteness
          : null;
      })
      .filter((v) => v != null);
    const overallValues = metrics
      .map((m) =>
        Number.isFinite(m.overallScore)
          ? m.overallScore
          : Number.isFinite(m.translationScore)
          ? m.translationScore
          : null
      )
      .filter((v) => v != null);
    ...
    summary.passCount = entries.filter(
      (entry) => entry.qa && entry.qa.gold_check === "pass"
    ).length;
    summary.averageCodeSwitchF1 = codeswitchValues.length
      ? average(codeswitchValues)
      : 0;
    ...
    summary.averageCueDiffSec = cueDiffValues.length
      ? average(cueDiffValues)
      : 0;
    summary.translationCompletenessAvg = translationCompletenessValues.length
      ? average(translationCompletenessValues)
      : 0;
    summary.translationCorrectnessAvg = translationCorrectnessValues.length
      ? average(translationCorrectnessValues)
      : 0;
    summary.translationCharRatioAvg = translationCharRatioValues.length
      ? average(translationCharRatioValues)
      : 0;
    ...
    const timeSpentValues = qaPayloads
      .map((qa) => (Number.isFinite(qa.time_spent_sec) ? qa.time_spent_sec : null))
      .filter((v) => v != null);
    summary.averageTimeSpentSec = timeSpentValues.length
      ? average(timeSpentValues)
      : 0;
    return summary;
  }
  ...
  function aggregateByAnnotator(entries) {
    ...
    return Array.from(perAnnotator.entries()).map(([annotator, list]) => {
      ...
      return Object.assign({ annotator_id: annotator }, summary, {
        clips: list.length,
      });
    });
  }
  ...
  function buildClipComment(clip, metrics, options) {
    ...
    if (breakdown.errorCount) {
      latestIssues.push(
        `${breakdown.errorCount} blocking ${
          breakdown.errorCount === 1 ? "issue" : "issues"
        }`
      );
    }
    if (breakdown.warningCount) {
      latestIssues.push(
        `${breakdown.warningCount} warning${
          breakdown.warningCount === 1 ? "" : "s"
        }`
      );
    }
    if (breakdown.translationGaps) {
      latestIssues.push(
        `${breakdown.translationGaps} translation gap${
          breakdown.translationGaps === 1 ? "" : "s"
        }`
      );
    }
    if (breakdown.codeSwitchIssues) {
      latestIssues.push(
        `${breakdown.codeSwitchIssues} code-switch flag${
          breakdown.codeSwitchIssues === 1 ? "" : "s"
        }`
      );
    }
  ...
  function generateReport(options) {
    ...
    // Optional "live" metrics using current annotations + manifest gold selection
    const annotations = opts.annotations || {};
    const goldClips = selectGoldClips(manifest);
    const liveMetrics = goldClips.length
      ? computeMetrics(goldClips, { annotations })
      : null;
    ...
    // Historical/aggregated metrics from local history
    ...
    const clips = entries.map((entry) => {
      ...
      const manifestInfo =
        findInManifest(manifest, clipId) || entry.clip || entry.item || {};
      const title =
        manifestInfo.title ||
        manifestInfo.clip_title ||
        manifestInfo.display ||
        clipId ||
        "Unknown clip";
      const language =
      ...
      return {
        ...
        metrics: {
          codeswitch_f1:
            qaPayload.codeswitch_f1 != null
              ? qaPayload.codeswitch_f1
              : metrics.codeswitch
              ? metrics.codeswitch.f1
              : null,
          diarization_mae:
            qaPayload.diarization_mae != null
              ? qaPayload.diarization_mae
              : metrics.diarization
              ? metrics.diarization.mae
              : null,
          ...
          translation_completeness:
            qaPayload.translation_completeness != null
              ? qaPayload.translation_completeness
              : metrics.translation
              ? metrics.translation.completeness
              : null,
          translation_char_ratio:
            qaPayload.translation_char_ratio != null
              ? qaPayload.translation_char_ratio
              : metrics.cues
              ? metrics.cues.translationCompleteness
              : null,
          translation_correctness: metrics.translation
            ? metrics.translation.correctness
            : null,
        },
        scores: {
    ...
    // Optional review comments for currently selected (gold) clips using live lint
    const review = liveMetrics
      ? {
          totalGoldClips: liveMetrics.totalGoldClips,
          reviewedClips: liveMetrics.reviewedClips,
          accuracyScore: liveMetrics.accuracyScore,
          consistencyScore: liveMetrics.consistencyScore,
          notes: liveMetrics.notes,
          breakdown: liveMetrics.breakdown,
          clipComments: goldClips.map((clip, index) => {
            const base = normalizeClip(clip, index);
            const status = buildClipComment(base, liveMetrics, {
              annotations,
              currentClipId,
            });
            return { ...base, ...status };
    ...
    const report = {
      ...
      annotator:
        opts.annotator ||
        (perAnnotator[0] && perAnnotator[0].annotator_id) ||
        "anonymous",
      summary,
      perAnnotator,
      clips,
